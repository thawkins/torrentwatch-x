<div class="dialog_window" id="show_legend">
    <ul id='torrentlist' class='show_legend'>
      <li class='legend match_nomatch'>
        <span class='torrent_name'><b>No Match</b> <br> <i>Will not be downloaded</i></span>
      <li class='legend match_duplicate'>
        <span class='torrent_name'><b>Duplicate Episode</b><br><i> Same episode downloaded previously.  Will not be downloaded.</i></span>
      <li class='legend match_old'>
        <span class='torrent_name'><b>Old Episode</b><br><i> Episode is Older than the most recently downloaded episode.</i></span>
      <li class='legend match_cachehit'>
        <span class='torrent_name'><b>Cache Hit - Download</b><br><i> This item is either downloading or downloaded.</i></span>
      <li class='legend match_old_download'>
        <span class='torrent_name'><b>Cache Hit - Old Download</b><br><i> This item was downloaded but is no longer in the torrent client.</i></span>
      <li class='legend match_test'>
        <span class='torrent_name'><b>Test Hit</b><br><i> This item is ready to download.  Press Refresh to start all test hits.</i></span>
      <li class='legend match_match'>
        <span class='torrent_name'><b>New Hit</b><br><i> This item has just been started.</i></span>
    </ul>
    <a class="toggleDialog button" href="#">Close</a>
</div>
